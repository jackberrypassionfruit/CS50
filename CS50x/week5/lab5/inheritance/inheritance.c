...
person *create_family(int generations)
{
    // TODO_DONE: Allocate memory for new person
    ...
    if (generations > 1)
    {
        ...
        // TODO_DONE: Set parent pointers for current person
...
void free_family(person *p)
{
    // TODO_DONE: Handle base case of freeing child (techinally grandparent lol)
    if ((*p).parents[0] == NULL)
    {
        free(p);
    }
    // TODO_DONE: Free parents recursively and only THEN free child
    else
    {
